...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        """
                ...
                使用tokenizer将文本转换为嵌入向量，然后通过添加位置嵌入来捕捉文本中的位置信息。
                通过Transformer模型对嵌入向量进行处理，以捕捉上下文依赖性。
                最后，通过线性变换得到最终的编码表示。

                参数:
                text: 待编码的文本，可以是单个字符串或字符串的列表。
        ...
        # 添加位置嵌入，以捕捉文本中单词的位置信息
        x = x + self.positional_embedding
        # 调整张量维度，以适应Transformer模型的输入要求
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x)
        ...
        # 从Transformer的输出中提取与每个文本对应的编码表示
